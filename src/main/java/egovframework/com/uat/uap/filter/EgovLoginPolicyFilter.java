...
public class EgovLoginPolicyFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (id == null || userSe == null) {
			((HttpServletResponse) response).sendRedirect(httpRequest.getContextPath() + "/cm/login/login.do");
		}
		...
		try {
			...
			if (loginPolicyYn) {
				...
				((HttpServletResponse) response).sendRedirect(httpRequest.getContextPath() + "/cm/login/login.do?message="+message);
			}

		} catch (IOException e) {//KISA 보안약점 조치 (2018-10-29, 윤창원)
			LOGGER.error("["+ e.getClass() +"] : ", e.getMessage());
			((HttpServletResponse) response).sendRedirect(httpRequest.getContextPath() + "/cm/login/login.do?login_error=1");
		} catch (Exception e) {
